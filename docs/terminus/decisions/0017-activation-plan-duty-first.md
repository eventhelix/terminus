# ADR-0017: Activation plan — light the duty ring, patch the holes, prune the overlap

Status: accepted
Date: 2026-08-25 (amended 2026-08-30: adoption changed from the unpruned to
the pruned duty-first policy — the prune pass removes the redundant lit
overlap where the polar rings converge at the poles)
...
satellites from other rings are added only where the band would otherwise
be unserved, and a final prune pass switches off any satellite whose every
point keeps its cover without it — recovering the satellites made redundant
by later picks, chiefly at the poles where the rings converge. The plan is
computed on the ground as a timetable (ADR-0009) and
...
**Duty-first + prune is chosen over the exact minimum** despite costing ~1.7
more satellites lit on average, because:

- It is explainable. An operator, and a reader, can state the rule in one
  sentence — light the block, patch the holes, switch off what turned out to
  serve no one — and the duty ring stays the organising idea of the
  architecture.
- It switches **two-fifths less often** (253/h against 423/h). Every switch
  is a thermal and power-electronics cycle, and the exact plan's freedom to
...
At any realistic warm-up the pruned and unpruned plans converge in mean lit —
but what the prune pass removes is not noise: it is the standing overlap
where the polar rings converge and a satellite lit by an early pick is made
redundant by a later one, most visibly at the poles. **The pruned policy is
adopted** (mean 23.1 lit, 32%, peak 30, 253 switches/h): the prune step is
one more line an operator can state, and it stops the fleet paying power and
interference for coverage the plan already has twice. Even at a ten-minute
lead the fleet runs under half lit.

## Consequences

- **Power, thermal and interference budgets size to ~35% mean and ~45% peak
  duty cycle**, not 100% — conservative roundings that hold for the pruned
  plan with its warm-up and hold margins. Peak lit is 30 of 72 before
  warm-up.
...
- **A hysteresis (lazy-off) hold is required, and is nearly free.** Planning
  ...
  the knee, at 180 s. (Those hold-sweep figures were measured on the unpruned
  baseline; the mechanism and the knee are the same for the pruned plan.)
  Implemented as `activation::smooth_schedule`.
