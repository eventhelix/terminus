...
| TER-REQ-008 | Cold start ≤15 min; reacquisition ≤30 s | Designed | ADR-0007; `first_contact` (tags post-9 through post-9c): X-band lantern (aperture-set Doppler spread, v·k/D — same ±6 kHz as Ka from a 3.57° beam), 13.3 s raster, 43 s worst case, 21× margin; warm start in seconds, storm-proof via X | — |
...
| TER-REQ-012 | Flare: degrade, never drop; alert ≤10 s | Partial | Band-fallback plan (ADR-0005) with a designed trigger: the X lantern doubles as the weather-recovery channel — a terminal whose Ka beam drowns answers the raster and requests sustained X service for its spot (ADR-0007); timetable-breathing FEC overhead architecture for flare/handover loss (ADR-0010); PHY residual-erasure contract 1%/5% (ADR-0011) | Codec machinery and burst-loss experiments (Series 2); PNT integrity alert (Series 3) |
