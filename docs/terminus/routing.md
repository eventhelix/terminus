...
state churns for nothing. The constellation explorer shows it at a wide
margin, which is unsurprising at a thousand times real speed: that is the
...
proposal quotes come from. Fifteen flips a day is one every ninety-odd
minutes; the transport absorbs a re-ordering that rare (ADR-0010), and
`select_anchor`'s margin already guards the expensive decision one level up.
If the margin is ever flown wide in service, this is the first thing to
revisit.
