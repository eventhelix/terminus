# helixsim Slice 1 — LEO Testbed Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A working `helixsim run <scenario.toml>` that simulates a 6-node LEO testbed (2 terminals, 3 satellites, 1 gateway) with real protocol bytes, producing per-node PCAPNG captures + matching Wireshark dissectors + metrics + config snapshot, byte-identical across runs with the same seed.
...
3. **Behaviors are a concrete enum** (`BehaviorKind`) inside `helixsim-core`, not a generic parameter on `NodeModel`. Reason: nexosim 1.0's `#[Model]` attribute macro is unverified on generic impl blocks, and `Model` requires `Serialize + DeserializeOwned` which trait objects break. The `NodeBehavior` trait still exists and each behavior implements it (that is the real-code-slot contract); `BehaviorKind` just delegates. Swapping in a real component later = new enum variant, or a generics refactor once macro support is confirmed.
4. **`helixsim-core` depends on `helixsim-protocols`** (behaviors encode/decode LinkFrames). protocols stays a leaf crate; no cycle.
...
**Interfaces:**
- Produces: `helixsim_core::rng::derive_seed(master: u64, path: &str) -> u64`, `helixsim_core::rng::model_rng(master: u64, path: &str) -> ChaCha12Rng` — used by every stochastic model and by assembly (Task 13).
...
helixsim-protocols = { path = "../protocols" }
```

(Keep the existing `[package]` block unchanged. The `helixsim-protocols` path dep is used from Task 11 on; adding it now is harmless — the crate already builds.)
...
Run: `cargo test -p helixsim-core`
...
Run: `cargo test -p helixsim-core trace`
...
Run: `cargo check -p helixsim-core`
...
Run: `cargo test -p helixsim-core compute`
...
Run: `cargo test -p helixsim-core medium`
...
Run: `cargo test -p helixsim-core netif`
...
Run: `cargo test -p helixsim-core capture`
...
- Produces (generated by pdl-compiler; exact field sets verified against 0.5.2):
  - `helixsim_protocols::link::{LinkFrame, DataFrame, ControlFrame, FrameType, LinkFrameChild}`
  ...
  - `helixsim_protocols::PdlPacket` (re-export of `pdl_runtime::Packet`: `encode_to_vec`, `decode_full`, ...)
...
```
// helixsim link framing (slice 1). One .pdl generates both the Rust
...
```toml
[package]
name = "helixsim-protocols"
description = "Protocol encoders/decoders for helixsim: PDL-generated codecs and Wireshark dissectors, plus etherparse-based IP/UDP/TCP builders."
...
# feature silences unexpected_cfgs. Not enabled by helixsim itself.
serde = ["dep:serde"]
...
Run: `cargo test -p helixsim-protocols`
Expected: 4 tests PASS. If generated field names differ from the Interfaces block above, fix THIS PLAN's later tasks to match reality (`cargo doc -p helixsim-protocols --no-deps` shows the generated API).
...
Run: `cargo test -p helixsim-protocols udp`
...
**Interfaces:**
- Produces: `crates/protocols/dissectors/link.lua` — a self-contained Wireshark plugin: `LinkFrame_protocol` (generated) + a `helixsim` wrapper Proto registered on `wtap.USER0` that chains DATA payloads (offset 14, see Task 8) to the built-in IP dissector. Task 13 embeds this file via `include_str!` and copies it into every run's `dissectors/` directory.
...
Expected: `crates/protocols/dissectors/link.lua` exists, contains `LinkFrame_protocol = Proto(` near the top, `DataFrame_match_constraints` (child dispatch — proof the `_body_` choice worked), and ends with the glue's `DissectorTable.get("wtap_encap"):add(wtap.USER0, helixsim_proto)`.
...
Run: `cargo test -p helixsim-protocols dissector`
Expected: 2 tests PASS.
...
**Interfaces:**
- Consumes: `Packet`/`PacketMeta`/`RxFrame` (Task 2), `MetricRecord` (Task 2), `helixsim_protocols::link::*` + `PdlPacket` (Task 8), `udp::*` (Task 9)
- Produces:
...
[package]
name = "helixsim"
description = "helixsim: an open-source Rust discrete-event simulator for networking and wireless systems, with real-bytes PCAP capture for Wireshark and VisualEther analysis."
version.workspace = true
edition.workspace = true
rust-version.workspace = true
license.workspace = true
...
Run: `cargo test -p helixsim config`
...
- Produces:
  ...
  - CLI: `helixsim run <scenario.toml> [--out out]` → `out/<scenario-name>/run-<unix-secs>/`
...
Run: `cargo run -p helixsim -- run crates/scenarios/leo-testbed/scenario.toml --out out`
Expected: prints the run dir; it contains `nodes/term-a.pcapng` (+5 more), `dissectors/link.lua`, `visualether.toml`, `metrics.ndjson` (non-empty), `scenario.snapshot.toml`. Open a pcapng in Wireshark if available locally — frames should dissect as helixsim → IP → UDP with `Data`.
...
Run: `cargo test -p helixsim --test smoke`
...
```bash
UPDATE_GOLDEN=1 cargo test -p helixsim --test golden
cargo test -p helixsim --test golden --test determinism
```
  ...
  test:
    ...
    steps:
      ...
      - name: tshark dissection acceptance
        run: cargo test -p helixsim --test dissection -- --ignored
```
...
Run: `cargo test --workspace` then (if tshark installed locally) `cargo test -p helixsim --test dissection -- --ignored`
